...
class HGVSRegex(object):
    ...
    CDNA_COORD = r"(?P<coord_prefix>|-|\*)(?P<coord>\d+)((?P<offset_prefix>-|\+)(?P<offset>\d+))?"
    CDNA_START = r"(?P<start>(?P<start_coord_prefix>|-|\*)(?P<start_coord>\d+)((?P<start_offset_prefix>-|\+)(?P<start_offset>\d+))?)"
    CDNA_END = r"(?P<end>(?P<end_coord_prefix>|-|\*)(?P<end_coord>\d+)((?P<end_offset_prefix>-|\+)(?P<end_offset>\d+))?)"
    ...
    # Peptide syntax
    PEP = r"(([A-Z]([a-z]{2}))|([GAVLIMFWPSTCYNQDEKRH\*]))+"
    PEP_REF = "(?P<ref>" + PEP + ")"
    PEP_REF2 = "(?P<ref2>" + PEP + ")"
    PEP_ALT = "(?P<alt>" + PEP + ")"

    PEP_EXTRA = r"(?P<extra>(|=|\?)(|fs((Ter|\*)\d+)?))"
